...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const loadState = () => {
    ...
    // 1) Tenta ler NEXT_PUBLIC_API_URL definida no painel da Vercel
    //    (deploySettings -> Environment Variables).
    // 2) Em dev local o Next injeta essa variavel em window.__ENV ou process.env,
    //    mas como aqui é HTML estatico, a Vercel injeta via next/script se existir.
    //    Solucao mais robusta para HTML estatico: buscar um arquivo /api-config.json
    //    Servido pela propria Vercel (gerado no build) OU fallback embutido.
    const API_URL_FROM_ENV =
      (typeof window !== "undefined" && window.__NEXT_PUBLIC_API_URL__)
      || "";

    // Padroniza valor inicial do campo de URL:
    //   - Se a Vercel injetou algo, usa ele.
    //   - Senao, tenta sessionStorage.
    //   - Senao, fallback relativo ("/conteudo" -> mesmo dominio).
    let initialUrl = API_URL_FROM_ENV || savedUrl || "/conteudo";
    // Garante que aponta para o endpoint /conteudo
    if (initialUrl && !initialUrl.endsWith("/conteudo")) {
      initialUrl = initialUrl.replace(/\/$/, "") + "/conteudo";
    }

    // Ativa modo live automaticamente quando a Vercel define a variavel
    const autoLive = Boolean(API_URL_FROM_ENV);

    if (apiToggle) {
      apiToggle.checked = autoLive || savedMode === "live";
    }
    if (apiUrlInput) {
      apiUrlInput.disabled = !(autoLive || savedMode === "live");
      apiUrlInput.value = initialUrl;
    }

    // Persiste o padrao para sessoes seguintes se o usuario nao mexer
    if (autoLive && !savedUrl) {
      sessionStorage.setItem("techmind_api_url", initialUrl);
      sessionStorage.setItem("techmind_api_mode", "live");
    }
  };

  // Funcao utilitaria que outras partes do app podem chamar para resolver a URL
  // final do backend, considerando o override do sessionStorage do usuario.
  window.__resolveApiUrl = function () {
    const saved = sessionStorage.getItem("techmind_api_url");
    const env = (typeof window !== "undefined" && window.__NEXT_PUBLIC_API_URL__) || "";
    let url = saved || env || "/conteudo";
    if (url && !url.endsWith("/conteudo")) {
